...
void PlotWidget::processPlotData()
{
	...
	{	QMessageBox::critical(this, "Plot Import", "An error occured while trying to import data of this plot.\n"
									"Please check this file for errors.",QMessageBox::Ok);
		clearPlotArea();
		return;
	}
...
{
	const QString fileName = QFileDialog::getSaveFileName( this, "Export File Name", QString(),	"PDF Documents (*.pdf)");
	if ( !fileName.isEmpty() )
	{
		QPrinter printer;
...
void PlotWidget::updateTitles(bool b)
{
	...
	if(b)
	{
		...
		if(chXTitle->isChecked() && (! m_plot_xlabel.isEmpty()) )    
			plotMain->setAxisTitle(QwtPlot::xBottom, QwtText(m_plot_xlabel));
